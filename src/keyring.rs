...
pub const KEYSTORE_DEFAULT_FOLDER: &str = "~/.keys/";
...
impl KeyStore for Test {
    fn get_key(&self, key_name: &str) -> Result<KeyEntry, Error> {
        ...
        let file = File::open(&key_file).map_err(|_| {
            Kind::KeyStore.context(format!("cannot open key file at '{}'", key_file.display()))
        })?;

        let key_entry = serde_json::from_reader(file).map_err(|e| {
            Kind::KeyStore.context(format!(
                "invalid key file at '{}': {}",
                key_file.display(),
                e
            ))
...
impl KeyRing {
    pub fn new(store: Store, account_prefix: &str, chain_id: &str) -> Result<Self, Error> {
        match store {
            Store::Memory => Ok(Self::Memory(Memory::new(account_prefix.to_string()))),

            Store::Test => {
                let keys_folder = disk_store_path(chain_id).map_err(|e| {
